...
public class DatabaseHelper extends NoLocaleSQLiteHelper implements
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        switch (oldVersion) {
            ...
            // Fall through
            case 32: {
                addTextColumn(db, Favorites.FILE_NAME, "");
                addTextColumn(db, Favorites.PACKAGE_NAME, "");
            }
            case 33:{
    ...
    private boolean addTextColumn(SQLiteDatabase db, String columnName, String defaultValue) {
        try (SQLiteTransaction t = new SQLiteTransaction(db)) {
            db.execSQL("ALTER TABLE favorites ADD COLUMN "
                    + columnName + " TEXT NOT NULL DEFAULT " + defaultValue + ";");
            t.commit();
        } catch (SQLException ex) {
            Log.e(TAG, ex.getMessage(), ex);
            return false;
